...
class TicketsSupportModuleFrontController extends ModuleFrontController
{
        ...
        if (Tools::getIsset('ticket') && Validate::isUnsignedInt(Tools::getValue('ticket'))) {
            ...
            if (Validate::isLoadedObject($thread)) {
                ...
                $this->context->smarty->assign(array(
                    ...
                    'reply_message' => count($this->errors)
                        ? Tools::nl2br(Tools::stripslashes(Tools::getValue('reply_message')))
                        : '',
                    'contact' => $contact,
                    'thread' => $thread
                ));
    ...
    {
        if (!($id_customer_thread = (int)Tools::getValue('ticket'))
        || !Validate::isUnsignedInt($id_customer_thread)) {
